...
If this proposal passes, the team instantiates the factory and router (two legacy factory config fields reference existing audited cw20-base/cw721-base code IDs; they are unused at runtime — no CW20 or NFT contracts are instantiated). Pools only ever hold OSMO, their own TokenFactory denom, and the gamm/pool LP shares of the pool they seeded; there is no privileged mint of OSMO and no bridged-asset exposure.
