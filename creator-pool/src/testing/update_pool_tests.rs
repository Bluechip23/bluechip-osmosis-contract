...
use crate::{
    ...
    state::{COMMIT_LIMIT_INFO, MAX_MIN_COMMIT},
...
/// <= `MAX_MIN_COMMIT`. The apply-side guard rejects zero on
/// `min_commit_pre_threshold` (defense-in-depth — factory's
...
fn test_update_config_rejects_zero_pre_threshold_floor() {
    ...
    match err {
        ContractError::InvalidCommitFloor { field, got, max } => {
            assert_eq!(field, "min_commit_pre_threshold");
            assert_eq!(got, Uint128::zero());
            assert_eq!(max, MAX_MIN_COMMIT);
        }
...
fn test_update_config_rejects_zero_post_threshold_floor() {
    ...
    let factory_info = message_info(&Addr::unchecked("factory_contract"), &[]);
    let update = PoolConfigUpdate {
        min_commit_post_threshold: Some(Uint128::zero()),
        ..Default::default()
    };
    ...
    match err {
        ContractError::InvalidCommitFloor { field, got, max } => {
            assert_eq!(field, "min_commit_post_threshold");
            assert_eq!(got, Uint128::zero());
            assert_eq!(max, MAX_MIN_COMMIT);
        }
        other => panic!("expected InvalidCommitFloor, got {:?}", other),
...
fn test_update_config_rejects_pre_floor_above_max() {
    ...
    let factory_info = message_info(&Addr::unchecked("factory_contract"), &[]);
    let too_high = MAX_MIN_COMMIT + Uint128::one();
    let update = PoolConfigUpdate {
        min_commit_pre_threshold: Some(too_high),
        ..Default::default()
    };
...
fn test_update_config_rejects_post_floor_above_max() {
    ...
    let too_high = MAX_MIN_COMMIT + Uint128::one();
    let update = PoolConfigUpdate {
        min_commit_post_threshold: Some(too_high),
        ..Default::default()
    };
...
/// Positive boundary: a value equal to `MAX_MIN_COMMIT` must succeed
...
fn test_update_config_accepts_floors_at_max_boundary() {
    ...
    let factory_info = message_info(&Addr::unchecked("factory_contract"), &[]);
    let update = PoolConfigUpdate {
        min_commit_pre_threshold: Some(MAX_MIN_COMMIT),
        min_commit_post_threshold: Some(MAX_MIN_COMMIT),
        ..Default::default()
    };
    ...
    assert_eq!(stored.min_commit_pre_threshold, MAX_MIN_COMMIT);
    assert_eq!(stored.min_commit_post_threshold, MAX_MIN_COMMIT);
}
