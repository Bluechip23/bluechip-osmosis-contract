...
/// Save a minimal `PoolDetails` for `pool_id` (plus the reverse
/// address->id index) so production code that resolves pools via
/// `POOLS_BY_ID` / `POOL_ID_BY_ADDRESS` works in these tests. Kept local
/// to this file so it doesn't depend on helpers owned by other test
/// modules.
